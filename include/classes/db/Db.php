...
abstract class Db
{
	...
	protected static $_servers = array(
		array('server' => _DB_SERVER_, 'user' => _DB_USER_, 'password' => _DB_PASS_, 'database' => _DB_NAME_), /* MySQL Master server */
	...
	public function autoExecute($table, $data, $type, $where = '', $limit = 0, $use_cache = true, $use_null = false)
	{
		...
		switch ($type)
		{
			...
			default :
				throw new RADatabaseException('Wrong argument (miss type) in Db::autoExecute()');
		}
	}
	...
	public function insert($table, $data, $null_values = false, $use_cache = true, $type = Db::INSERT, $add_prefix = true)
	{
		...
		if ($type == Db::INSERT)
			...
			throw new RADatabaseException('Bad keyword, must be Db::INSERT or Db::INSERT_IGNORE or Db::REPLACE');
			...
			foreach ($row_data as $key => $value)
			{
				if (isset($keys_stringified))
				{
					...
					if (!in_array("`$key`", $keys))
						throw new RADatabaseException('Keys form $data subarray don\'t match');
				}
				else
					$keys[] = "`$key`";
	...
	public function executeS($sql, $array = true, $use_cache = true)
	{
		...
		if (!preg_match('#^\s*\(?\s*(select|show|explain|describe|desc)\s#i', $sql))
		{
			if (defined('_PS_MODE_DEV_') && _PS_MODE_DEV_)
				throw new RADatabaseException('Db->executeS() must be used only with select, show, explain or describe queries');
	...
	public function displayError($sql = false)
	{
		...
		{
			if ($sql)
				throw new RADatabaseException($this->getMsgError().'<br /><br /><pre>'.$sql.'</pre>');
			throw new RADatabaseException($this->getMsgError());
		}
	}
